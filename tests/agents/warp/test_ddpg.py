...
from skrl.resources.noises.warp import GaussianNoise, OrnsteinUhlenbeckNoise
...
from skrl.utils.model_instantiators.warp import deterministic_model
...
@hypothesis.given(
    ...
    learning_rate_scheduler=st.one_of(st.none(), st.just(KLAdaptiveLR)),
...
def test_agent(
    ...
    if not is_device_available(device, backend="warp"):
