...
class Memory(ABC):
    ...
    def get_tensor_by_name(self, name: str) -> wp.array:
    ...
    def add_samples(self, **tensors: Mapping[str, wp.array]) -> None:
        ...
        * ``current_num_envs > num_envs`` and ``num_envs = 1``: store multiple samples and increment the memory index
        ...
        # multi environment (current_num_envs < num_envs)
            ...
            for name, tensor in tensors.items():
                if name in self.tensors and tensor is not None:
                    wp.copy(self.tensors[name][self.memory_index, self.env_index : self.env_index + shape[0]], tensor)
        ...
        # single environment (current_num_envs = 1, implicit)
            ...
            for name, tensor in tensors.items():
                if name in self.tensors and tensor is not None:
                    wp.copy(self.tensors[name][self.memory_index, self.env_index], tensor)
            self.env_index += 1
        else:
            raise ValueError(
