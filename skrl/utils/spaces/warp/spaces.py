...
def tensorize_space(
    ...
    :param device: Device on which a tensor/array is or will be allocated.
        This parameter is used when the space value is not a Warp array (e.g.: NumPy array, number).
    ...
    if space is None or x is None:
        return None
    device = config.warp.parse_device(device)
    # fundamental spaces
    # - Box
    if isinstance(space, spaces.Box):
        if isinstance(x, wp.array):
            ...
            return wp.array(x, device=device, dtype=wp.float32).reshape((-1, *space.shape))
        else:
    ...
    # - Discrete
        ...
        if isinstance(x, wp.array):
            ...
            return wp.array(x, device=device, dtype=wp.int32).reshape((-1, 1))
    ...
    # - MultiDiscrete
        ...
        if isinstance(x, wp.array):
            ...
            return wp.array(x, device=device, dtype=wp.int32).reshape((-1, *space.shape))
...
def flatten_tensorized_space(x: Any) -> Optional[wp.array]:
    ...
    """
    # TODO: ensure all arrays to concatenate have the same dtype (RuntimeError: Incompatible array data types)
    if x is None:
        return None
...
def unflatten_tensorized_space(space: Optional[Union[spaces.Space, Sequence[int], int]], x: Optional[wp.array]) -> Any:
    ...
    # - Box
    if isinstance(space, spaces.Box):
        return x.contiguous().reshape((-1, *space.shape))
    # - Discrete
    elif isinstance(space, spaces.Discrete):
        return x.contiguous().reshape((-1, 1))
    # - MultiDiscrete
    elif isinstance(space, spaces.MultiDiscrete):
        return x.contiguous().reshape((-1, *space.shape))
    # composite spaces
...
def compute_space_limits(
    ...
    *,
    ...
    device = config.warp.parse_device(device)
    if low is not None:
        low = wp.array(low, device=device)
    if high is not None:
        high = wp.array(high, device=device)
    return low, high
...
def sample_space(
    ...
    *,
    ...
    :param device: Device on which a tensor/array is or will be allocated.
        This parameter is used when the backend is ``"native"`` (Warp).
    ...
    if isinstance(space, spaces.Box):
        ...
        if backend == "numpy":
            ...
            return wp.array(sample, device=device).reshape((batch_size, *space.shape))
        else:
    ...
    # - Discrete
        ...
        if backend == "numpy":
            ...
            return wp.array(sample, device=device).reshape((batch_size, -1))
        else:
            raise ValueError(f"Unsupported backend type ({backend})")
    # - MultiDiscrete
    elif isinstance(space, spaces.MultiDiscrete):
        ...
        if backend == "numpy":
            ...
            return wp.array(sample, device=device).reshape((batch_size, *space.nvec.shape))
        else:
            raise ValueError(f"Unsupported backend type ({backend})")
    # composite spaces
    # - Tuple
