    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
            actions, log_prob, outputs = self.policy.act(
                {"states": self._state_preprocessor(states), "alpha": self._alpha, **rnn}, role="policy"
            )
        ...
        if self.memory is not None:
            ...
            with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                ...
                values, _, outputs = self.value.act(
                    {"states": self._state_preprocessor(states), "alpha": self._alpha, **rnn}, role="value"
                )
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        with torch.no_grad(), torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
            ...
            last_values, _, _ = self.value.act(
                {"states": self._state_preprocessor(self._current_next_states.float()), "alpha": self._alpha, **rnn},
                role="value",
            )
            ...
            for i, (
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    predicted_values, _, _ = self.value.act(
                        {"states": sampled_states, "alpha": self._alpha, **rnn_value}, role="value"
                    )
