...
class Shared(MultiCategoricalMixin, DeterministicMixin, Model):
    def __init__(
        self, observation_space, state_space, action_space, device, unnormalized_log_prob=True, reduction="sum"
    ):
        Model.__init__(
            self,
        ...
        self.net_pos = nn.Sequential(
            nn.Linear(compute_space_size(self.observation_space["joint-positions"]), 16),
            nn.ELU(),
            nn.Linear(16, 16),
            nn.ELU(),
        )
        self.net_vel = nn.Sequential(
            nn.Linear(compute_space_size(self.observation_space["joint-velocities"]), 16),
            nn.ELU(),
            nn.Linear(16, 16),
            nn.ELU(),
        )
    ...
    def act(self, inputs, role):
        if role == "policy":
            return MultiCategoricalMixin.act(self, inputs, role=role)
        elif role == "value":
    ...
    def compute(self, inputs, role):
        ...
        if role == "policy":
            self._shared_output = self.net_pos(observations["joint-positions"]) + self.net_vel(
                observations["joint-velocities"]
            )
            return self.policy_layer(self._shared_output), {}
        elif role == "value":
            shared_output = (
                self.net_pos(observations["joint-positions"]) + self.net_vel(observations["joint-velocities"])
...
task_name = "Isaac-Cartpole-Showcase-Dict-MultiDiscrete-Direct-v0"
